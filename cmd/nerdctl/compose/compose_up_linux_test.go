...
func TestComposeImageVolumeServiceNameIsLiteral(t *testing.T) {
	testCase := nerdtest.Setup()
	testCase.Require = nerdtest.Private
	...
	testCase.Setup = func(data test.Data, helpers test.Helpers) {
		...
		sourceName := data.Identifier("image-service-source-image")
		composeYAML := fmt.Sprintf(`
services:
  %s:
    image: %s
  ...
  app:
    ...
    volumes:
      - type: image
        source: %s
        target: /website
`, sourceName, testutil.NginxAlpineImage, testutil.CommonImage, containerName, sourceName)
		composePath := data.Temp().Save(composeYAML, "compose.yaml")
		...
		data.Labels().Set("containerName", containerName)
		data.Labels().Set("projectName", projectName)
		data.Labels().Set("sourceName", sourceName)
	}

	testCase.Command = func(data test.Data, helpers test.Helpers) test.TestableCommand {
		helpers.Command("compose", "-p", data.Labels().Get("projectName"), "-f", data.Labels().Get("composeYAML"), "up", "-d").Run(&test.Expected{
			ExitCode: expect.ExitCodeGenericFail,
			Errors:   []error{errors.New(data.Labels().Get("sourceName"))},
		})
