...
func testRename(t *testing.T, testImage string) {
	t.Parallel()
	testContainerName := testutil.Identifier(t)
	base := testutil.NewBase(t)

	defer base.Cmd("rm", "-f", testContainerName).Run()
	base.Cmd("run", "-d", "--name", testContainerName, testImage, "sleep", "infinity").AssertOK()
...
func testRenameUpdateHosts(t *testing.T, testImage string) {
	t.Parallel()
