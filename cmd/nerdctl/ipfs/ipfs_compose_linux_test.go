...
services:
  ...
  wordpress:
    ...
    restart: always
    ports:
      # FIXME: this is really bad and likely to collide with other tests
      - 8080:80
    environment:
...
volumes:
  wordpress:
  db:
`, data.Get(wordpressImageCIDKey), data.Get(mariaImageCIDKey)), data.Get(composeExtraKey))
		return helpers.Command("info")
	}

	testCase.Expected = test.Expects(0, nil, test.Equals("hello\n"))
