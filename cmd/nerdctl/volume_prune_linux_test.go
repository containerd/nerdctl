...
func TestVolumePrune(t *testing.T) {
	...
	subTearUp := func(tID string) {
		...
		base.Cmd("run", "--name", tID, "-v", tID+"-busy:/whatever", "-v", anonIDBusy, testutil.CommonImage).AssertOK()
	}
	...
	subTearDown := func(tID string) {
		...
		// base.Cmd("volume", "rm", "-f", tID+"-free").Run()
		// base.Cmd("volume", "rm", "-f", anonIDFree).Run()
		// base.Cmd("volume", "rm", "-f", anonIDBusy).Run()
	...
	testCases := []struct {
		...
		inspect     func(tID string, stdout string)
	}{
		{
			description: "prune ano",
			command: func(tID string) *testutil.Cmd {
				base := testutil.NewBaseWithNamespace(t, tID)
			...
			inspect: func(tID string, stdout string) {
				...
				base.Cmd("inspect", tID+"-free").AssertOK()
				assert.Assert(base.T, !strings.Contains(stdout, tID+"-busy"))
				base.Cmd("inspect", tID+"-busy").AssertOK()
		...
		},
		{
			...
			inspect: func(tID string, stdout string) {
				...
				base.Cmd("inspect", tID+"-free").AssertFail()
				// TODO verify the anonymous volumes status
			},
		},
	}
