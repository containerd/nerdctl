...
`
	...
	if err := container.Prune(ctx, cmd, client); err != nil {
		return err
	}
	if err := network.Prune(ctx, cmd, client); err != nil {
		return err
	}
	if vFlag {
		if err := volume.Prune(ctx, cmd, client); err != nil {
			return err
		}
	}
	return image.Prune(ctx, cmd, client)
}
