				...
				return &test.Expected{
					...
					Output: expect.All(func(stdout string, t tig.T) {
						...
						checkSystemdTimers := func(label string) {
							...
							// Check all timers - use Anyhow which doesn't fail the test on non-zero exit
							helpers.Anyhow("systemctl", "list-timers", "--no-pager")
							t.Log("Attempted to list all systemd timers")

							// Check specifically for nerdctl healthcheck timers
							helpers.Anyhow("systemctl", "list-timers", "--no-pager", "*nerdctl*")
							t.Log("Attempted to list nerdctl healthcheck timers")
						}
