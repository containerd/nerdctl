...
// absolute symlink (-> /etc, which exists on any host) is rejected, not followed
...
`, testutil.CommonImage)
	...
	testCase.Command = func(data test.Data, helpers test.Helpers) test.TestableCommand {
		return helpers.Command("run", "--rm",
			"--mount", fmt.Sprintf("type=image,source=%s,destination=/mnt/img,image-subpath=abs", data.Identifier("img")),
			testutil.CommonImage, "true")
	}

	testCase.Expected = func(data test.Data, helpers test.Helpers) *test.Expected {
		errMsg := "path escapes from parent"
		if nerdtest.IsDocker() {
			errMsg = "escapes the base directory"
		}
		return &test.Expected{
			ExitCode: expect.ExitCodeGenericFail,
			Errors:   []error{errors.New(errMsg)},
		}
	}
