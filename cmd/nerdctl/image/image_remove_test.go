...
func TestRemoveKubeWithKubeHideDupe(t *testing.T) {
	...
	testCase.SubTests = []*test.Case{
		...
		{
			Description: "Test multiple IDs found with provided prefix",
			NoParallel:  true,
			Setup: func(data test.Data, helpers test.Helpers) {
				helpers.Ensure("pull", testutil.BusyboxImage)
				helpers.Ensure("tag", testutil.BusyboxImage, data.Identifier())
			...
			Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
				return &test.Expected{
					Output: func(stdout string, info string, t *testing.T) {
						...
						helpers.Command("--kube-hide-dupe", "rmi", "debian").Run(&test.Expected{
							ExitCode: 1,
							Errors:   []error{errors.New("no such image: ")},
						})
						helpers.Command("images").Run(&test.Expected{
							Output: func(stdout string, info string, t *testing.T) {
								lines := strings.Split(strings.TrimSpace(stdout), "\n")
								assert.Assert(t, len(lines) == numNoTags + 3, info)
							},
						})
					},
				}
