		...
		SubTests: []*test.Case{
			{
				...
				Command:     test.RunCommand("image", "history", "--human=true", "--format=json", testutil.CommonImage),
			...
			},
			{
				...
				Command:     test.RunCommand("image", "history", "--human=false", "--format=json", testutil.CommonImage),
				Expected: test.Expects(0, nil, func(stdout string, info string, t *testing.T) {
					history, err := decode(stdout)
					assert.NilError(t, err, info)
			...
			},
			{
				...
				Command:     test.RunCommand("image", "history", "--human=false", "--no-trunc", "--format=json", testutil.CommonImage),
				Expected: test.Expects(0, nil, func(stdout string, info string, t *testing.T) {
					history, err := decode(stdout)
					assert.NilError(t, err, info)
			...
			},
			{
				...
				Command:     test.RunCommand("image", "history", "--human=false", "--no-trunc", "--quiet", testutil.CommonImage),
				Expected: test.Expects(0, nil, func(stdout string, info string, t *testing.T) {
