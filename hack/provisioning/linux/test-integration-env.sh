...
readonly root
# lib.sh requires shasum at source time, which EL does not ship by default
if ! command -v shasum >/dev/null && command -v dnf >/dev/null; then
  dnf install -q -y perl-Digest-SHA
fi
...
host::packages(){
  if command -v apt-get >/dev/null; then
    ...
    # `container-selinux` provides the contexts (container_t, ...) applied by the SELinux
    # tests - the labels are applied even in permissive mode
    dnf install -q -y \
      container-selinux \
      criu \
      expect \
...
host::configuration(){
  ...
  # Test-specific systemd units (started explicitly by host::services)
  ...
  # Preload the kernel modules needed by the tests: br-netfilter ensures that bridged
  # traffic goes through netfilter, and rootless containerd cannot trigger module
  # auto-loading from inside a user namespace. The list mirrors Lima's 00-modprobe.sh
  # boot script, which does not run for guests started in plain mode.
  local module
  for module in \
    br_netfilter \
    fuse \
    ...
    overlay; do
    modprobe "$module" || log::warning "Failed to load the $module module (negligible if it is built-in the kernel)"
  done

  # Ensure securityfs is mounted, so that AppArmor detection works
  mountpoint -q /sys/kernel/security || mount -t securityfs securityfs /sys/kernel/security
  # Load the "nerdctl-default" AppArmor profile for TestRunApparmor: in rootless mode,
  # the tests cannot load it themselves
...
EOT
  ...
  # Some tests publish ports 80 and 443, which the rootlesskit port driver has to bind
  # as the unprivileged user. The containerized test environment used to get this for
  # free: Docker sets this sysctl to 0 inside containers.
  sysctl -w net.ipv4.ip_unprivileged_port_start=0 >/dev/null

  # Allow unprivileged ICMP Echo sockets (EL disables them by default; this mirrors
  # Lima's 20-rootless-base.sh boot script, which does not run for guests started in
  # plain mode)
  sysctl -w "net.ipv4.ping_group_range=0 2147483647" >/dev/null

  # Without slirp4netns installed, rootlesskit v3.0+ falls back to its experimental
  # gvisor-tap-vsock network driver: always provide the stable slirp4netns driver.
  # slirp4netns is also required by the slirp4netns port driver, and by rootlesskit prior to v3.0.
  host::slirp4netns
}
...
provision::test-integration-env::install(){
  ...
  # This script substantially and irreversibly modifies the host it runs on:
  # it is only safe to run on a disposable CI machine
  [ "${GITHUB_ACTIONS:-}" == "true" ] || {
    log::error "Refusing to run outside of GitHub Actions (export GITHUB_ACTIONS=true to force)"
    return 1
  }
