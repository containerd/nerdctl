...
. "$root"/../scripts/lib.sh
. "$root"/logs-collector.sh

##### Configuration
# Where to store report files
readonly report_location=~/nerdctl-test-report
...
# DEPRECATED/FIXME: not working on windows. Need to change approach: move away from --post-run-command and
# just process the log file. This might also allow multi-steps/multi-target results aggregation.
[ "$(uname -s)" != "Linux" ] || args+=(--post-run-command "$root"/gotestsum-post-run.sh)
# Log file
...
collect::logs > "$report_location"/logs.tar
