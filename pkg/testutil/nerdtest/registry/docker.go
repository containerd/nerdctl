...
func NewDockerRegistry(data test.Data, helpers test.Helpers, t *testing.T, currentCA *ca.CA, port int, auth Auth) *Server {
	...
	cleanup := func(data test.Data, helpers test.Helpers) {
		...
		if cert != nil {
			assert.NilError(t, cert.Close(), fmt.Errorf("failed cleaning certificates: %w", err))
		}
	...
	hostsDir, err := func() (string, error) {
		hDir, err := os.MkdirTemp(t.TempDir(), "certs.d")
		assert.NilError(t, err, fmt.Errorf("failed creating directory certs.d: %w", err))
		...
		if currentCA != nil {
			...
			assert.NilError(t, err, fmt.Errorf("failed creating hosts.toml file: %w", err))
	...
	setup := func(data test.Data, helpers test.Helpers) {
		...
		_, err = nettestutil.HTTPGet(fmt.Sprintf("%s://%s/v2",
			...
			10,
			true)
		assert.NilError(t, err, fmt.Errorf("failed starting docker registry in a timely manner: %w", err))
	}
