		...
		cmd.Run(&test.Expected{
			Output: func(stdout string, info string, t *testing.T) {
				...
				if count == maxRetry-1 {
					...
					// This is just bad
					com := helpers.Command("logs", con)
					com.WithWrapper("unbuffer")
...
func NewCesantaAuthServer(data test.Data, helpers test.Helpers, t *testing.T, ca *ca.CA, port int, user, pass string, tls bool) *TokenAuthServer {
	...
	assert.NilError(t, err, fmt.Errorf("failed bcrypt encrypting password: %w", err))
	...
	assert.NilError(t, err, fmt.Errorf("failed creating temporary directory for config file: %w", err))
	...
	assert.NilError(t, err, fmt.Errorf("failed writing configuration: %w", err))
	...
	cleanup := func(data test.Data, helpers test.Helpers) {
		...
		if errPortRelease != nil {
			t.Error(errPortRelease.Error())
		}
		if errCertClose != nil {
			t.Error(errCertClose.Error())
		}
		if errConfigClose != nil {
			t.Error(errConfigClose.Error())
		}
		if errConfigRemove != nil {
			t.Error(errConfigRemove.Error())
		}
	}
	...
	setup := func(data test.Data, helpers test.Helpers) {
		...
		_, err = nettestutil.HTTPGet(fmt.Sprintf("%s://%s/auth",
			...
			10,
			true)
		assert.NilError(t, err, fmt.Errorf("failed starting auth container in a timely manner: %w", err))
