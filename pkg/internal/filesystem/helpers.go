...
func markerLocation(path string) string {
	...
	return filepath.Join(holdLocation, hash(dir)+"-"+pretty+"-"+hash(base)+"-"+markerSuffix)
}
...
func backupLocation(path string) string {
	...
	return filepath.Join(holdLocation, hash(dir)+"-"+pretty+"-"+hash(base)+"-"+backupSuffix)
}
...
func markerCreate(path string) (err error) {
	...
	marker, err = os.OpenFile(markerLocation(path), os.O_CREATE|os.O_WRONLY|os.O_TRUNC, privatePerm)
	if err != nil {
		return err
	}
	...
	defer func() {
		if err = errors.Join(err, marker.Close()); err != nil {
			err = errors.Join(err, os.Remove(markerLocation(path)))
		}
		time.Sleep(10 * time.Millisecond)
	}()
...
func internalCopy(sourcePath, destinationPath string) (err error) {
	...
	// Open source
	source, err = os.OpenFile(sourcePath, os.O_RDONLY, privatePerm)
	if err != nil {
		return err
	}
	...
	return fileWrite(source, srcInfo.Size(), destinationPath, privatePerm, srcInfo.ModTime())
}
