...
func TestApplyChanges(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "ENV value expands an earlier ENV",
			changes: []string{"ENV FOO=bar", "ENV B=$FOO"},
			want:    ocispec.ImageConfig{Env: []string{"FOO=bar", "B=bar"}},
		},
		{
			name:    "ENV unset variable expands to empty",
			changes: []string{"ENV B=x$NOPE.y"},
			want:    ocispec.ImageConfig{Env: []string{"B=x.y"}},
		},
		{
			name:    "LABEL pairs",
...
func TestApplyChangesErrors(t *testing.T) {
	testCases := []struct {
		...
		{"unknown instruction", "NOSUCHTHING foo", "unknown instruction"},
		{"run unsupported", "RUN echo hi", "not supported by import"},
		{"copy unsupported", "COPY a b", "not supported by import"},
		{"from unsupported", "FROM alpine", "not supported by import"},
		...
		{"label without value", "LABEL a=1 b", "can't find = in"},
		{"env legacy without a value", "ENV LONELYKEY", "two arguments"},
		{"env unterminated quote", `ENV A="oops`, "matching double-quote"},
		{"a change may hold only one instruction", "ENV A=b\nRUN touch /x", "single instruction"},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			var cfg ocispec.ImageConfig
