...
func Inspect(ctx context.Context, client *containerd.Client, identifiers []string, options types.ImageInspectOptions) ([]interface{}, error) {
	// Set a timeout
	...
	if len(errs) > 0 {
		return []interface{}{}, fmt.Errorf("%d errors:\n%w", len(errs), errors.Join(errs...))
	}
