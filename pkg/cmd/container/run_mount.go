...
// view's snapshot key, and a host materialization path (set only for a subpath).
...
// setupImageSubpathMount materializes the view on a host dir and returns a
// read-only bind mount of the subpath, resolved so it cannot escape the rootfs.
// On failure it unwinds the mount, dir, and view; the host path is removed on
// container delete.
func setupImageSubpathMount(ctx context.Context, options types.ContainerCreateOptions, ref, destination, subpath, snapshotKey string, mounts []mount.Mount, removeView func()) (specs.Mount, string, string, error) {
	...
	resolved, err := resolveImageSubpath(hostMountpoint, subpath)
	if err != nil {
		cleanup()
...
// resolveImageSubpath returns subpath under the materialized rootfs, with symlinks
// already resolved, for use as a bind source. mount(2) resolves the source against
// the host root, so an unresolved path would let an absolute symlink in the image
// bind the host's copy of the target instead of the image's.
func resolveImageSubpath(rootfs, subpath string) (string, error) {
	// securejoin follows symlinks relative to rootfs, so the result stays inside it.
	resolved, err := securejoin.SecureJoin(rootfs, subpath)
	if err != nil {
		return "", err
	}
	// OpenInRoot is the kernel's own scoped lookup, and is stricter: it confirms
	// the subpath exists and rejects any absolute symlink or escape rather than
	// re-anchoring it at rootfs. Docker rejects those subpaths too.
